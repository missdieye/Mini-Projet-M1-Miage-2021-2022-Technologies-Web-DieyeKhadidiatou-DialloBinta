...
exports.countRestaurants = async (name) => {
  ...
  try {
    if (name == "") {
      reponse = await db.collection("restaurants").countDocuments();
    } else {
      let query = {
        name: { $regex: ".*" + name + ".*", $options: "i" },
      };
      reponse = await db.collection("restaurants").find(query).count();
    }
  } catch (err) {
    reponse = {
      succes: false,
...
exports.findRestaurants = async (page, pagesize, name) => {
  ...
  try {
    ...
    if (name == "") {
      ...
      count = await db.collection("restaurants").find(query).count();
    }
...
exports.createRestaurant = async (formData) => {
  ...
  try {
    let toInsert = {
      ...
      "address":{"street" : formData.street ,"building":1}
    };
...
exports.deleteRestaurant = async function (id, callback) {
  ...
  try {
    ...
    let result = await db.collection("restaurants").deleteOne(myquery);
    reponse = {
      succes: true,
      result: result,
      error: null,
